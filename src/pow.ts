...
export function solvePow(challenge: PowChallenge): PowResponse {
  ...
  if (!Number.isSafeInteger(challenge.difficulty) || challenge.difficulty <= 0) throw new Error('Invalid PoW difficulty')
  const prefix = `${challenge.salt}_${challenge.expire_at}_`
  for (let answer = 0; answer < challenge.difficulty; answer += 1) {
    if (hash(`${prefix}${answer}`) === challenge.challenge) return { ...challenge, answer }
  }
...
function hash(input: string): string {
  ...
  for (let offset = 0; offset < bytes.length; offset += 136) {
    ...
    for (let index = 0; index < remaining; index += 1) state[index >>> 2] ^= bytes[offset + index] << ((index & 3) * 8)
    if (remaining < 136) {
      state[remaining >>> 2] ^= 0x06 << ((remaining & 3) * 8)
      state[33] ^= 0x80000000
    } else keccakP(state, rhoPi, parity, theta)
  }
  ...
  return Array.from({ length: 32 }, (_, index) => ((state[index >>> 2] >>> ((index & 3) * 8)) & 0xff).toString(16).padStart(2, '0')).join('')
}
...
function keccakP(state: Uint32Array, rhoPi: Uint32Array, parity: Uint32Array, theta: Uint32Array): void {
  for (let round = 1; round < 24; round += 1) {
    ...
    rhoPi[0] = state[0]; rhoPi[1] = state[1]
    for (let lane = 1; lane < 25; lane += 1) {
      ...
      const low = state[source]; const high = state[source + 1]
      if (amount < 32) {
